...
for time_pd in date_range:
    ...
    if duration > 24:
        #stage4_slice = stage4.sel(time=slice(time_pd - pd.Timedelta(hours=duration-1),time_pd)).tp.sum(dim='time')
        ...
        #stage4_slice = stage4.sel(time=time_pd).tp
        try:
            mrms_slice = mrms.sel(time=time_pd).tp.where(prism_mask>=0)
        #prism_slice = prism.sel(time=time_pd).tp
        except:
            print("MRMS missing on this date, moving on")
            continue
    elif duration < 24:
        print("subdaily not yet available")

    ### find the exceedances on this date
    mrms_exceed10 = mrms_slice.where((mrms_slice > ari10_grid.precip).compute(), drop=True).to_dataset()
    mrms_exceed100 = mrms_slice.where((mrms_slice > ari100_grid.precip).compute(), drop=True).to_dataset()    
    mrms_exceed1000 = mrms_slice.where((mrms_slice > ari1000_grid.precip).compute(), drop=True).to_dataset()

    ### get 10-year exceedances into a dataframe
    if (mrms_exceed10.tp.shape[0] > 0): ### if there are events
        mrms_exceed10_df = create_exceed_df(mrms_exceed10,ari10_grid,time_pd)

    ### repeat for 100
    if (mrms_exceed100.tp.shape[0] > 0): ### if there are events
        mrms_exceed100_df = create_exceed_df(mrms_exceed100,ari100_grid,time_pd)
    else:
        mrms_exceed100_df = None

    ### repeat for 1000
    if (mrms_exceed1000.tp.shape[0] > 0): ### if there are events
        mrms_exceed1000_df = create_exceed_df(mrms_exceed1000,ari1000_grid,time_pd)
    else:
        mrms_exceed1000_df = None
    ...
    if (mrms_exceed10.tp.shape[0] > 0): ### if there are events
        df_clustered, n_clusters = cluster_points_dbscan(mrms_exceed10_df, max_distance_km=350, min_points=1)
        ...
        for j in range(0,n_clusters):
            ...
            #else:
            #    stage4_this = stage4.sel(time=time_pd,lon=slice(minlon,maxlon),lat=slice(minlat,maxlat)).tp
            ...
            data1_exceed10 = np.isfinite(mrms_exceed10_ds.values)
            data2_exceed10 = np.isfinite(prism_exceed10_ds.values)
            data1_exceed100 = np.isfinite(mrms_exceed100_ds.values)
            data2_exceed100 = np.isfinite(prism_exceed100_ds.values)
            data1_exceed1000 = np.isfinite(mrms_exceed1000_ds.values)
            data2_exceed1000 = np.isfinite(prism_exceed1000_ds.values)
            ...
            removed10_df_this = parse_latlons(mrms_this,results['removed_data1_exceed10'],mrms_exceed10_df,event_num,time_pd)
            ...
            if mrms_exceed100_df is not None:
                exceed100_df_this = parse_latlons(mrms_this,results['data1_exceed100'],mrms_exceed100_df,event_num,time_pd)
                exceed100_df_all = pd.concat([exceed100_df_all,exceed100_df_this])
                removed100_df_this = parse_latlons(mrms_this,results['removed_data1_exceed100'],mrms_exceed100_df,event_num,time_pd)
                removed100_df_all = pd.concat([removed100_df_all,removed100_df_this])

            if mrms_exceed1000_df is not None:
                exceed1000_df_this = parse_latlons(mrms_this,results['data1_exceed1000'],mrms_exceed1000_df,event_num,time_pd)
                exceed1000_df_all = pd.concat([exceed1000_df_all,exceed1000_df_this])
                removed1000_df_this = parse_latlons(mrms_this,results['removed_data1_exceed1000'],mrms_exceed1000_df,event_num,time_pd)
