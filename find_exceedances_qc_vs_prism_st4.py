...
for time_pd in date_range:
    ...
    stage4_exceed10 = stage4_slice.where((stage4_slice > ari10_grid.precip).compute(), drop=True).to_dataset()
    stage4_exceed100 = stage4_slice.where((stage4_slice > ari100_grid.precip).compute(), drop=True).to_dataset()    
    stage4_exceed1000 = stage4_slice.where((stage4_slice > ari1000_grid.precip).compute(), drop=True).to_dataset()

    ### get 10-year exceedances into a dataframe
    if (stage4_exceed10.tp.shape[0] > 0): ### if there are events
        stage4_exceed10_df = create_exceed_df(stage4_exceed10,ari10_grid,time_pd)

    ### repeat for 100
    if (stage4_exceed100.tp.shape[0] > 0): ### if there are events
        stage4_exceed100_df = create_exceed_df(stage4_exceed100,ari100_grid,time_pd)
    else:
        stage4_exceed100_df = None

    ### repeat for 1000
    if (stage4_exceed1000.tp.shape[0] > 0): ### if there are events
        stage4_exceed1000_df = create_exceed_df(stage4_exceed1000,ari1000_grid,time_pd)
    else:
        stage4_exceed1000_df = None
    ...
    if (stage4_exceed10.tp.shape[0] > 0): ### if there are events
        df_clustered, n_clusters = cluster_points_dbscan(stage4_exceed10_df, max_distance_km=350, min_points=1)
...
#            minlon = this_lon - lon_rad
#            maxlon = this_lon + lon_rad
#            minlat = this_lat - lat_rad
#            maxlat = this_lat + lat_rad
            ...
            data1_exceed10 = np.isfinite(stage4_exceed10_ds.values)
            data2_exceed10 = np.isfinite(prism_exceed10_ds.values)
            data1_exceed100 = np.isfinite(stage4_exceed100_ds.values)
            data2_exceed100 = np.isfinite(prism_exceed100_ds.values)
            data1_exceed1000 = np.isfinite(stage4_exceed1000_ds.values)
            ...
            removed10_df_this = parse_latlons(stage4_this,results['removed_data1_exceed10'],stage4_exceed10_df,event_num,time_pd)
            ...
            if stage4_exceed100_df is not None:
                exceed100_df_this = parse_latlons(stage4_this,results['data1_exceed100'],stage4_exceed100_df,event_num,time_pd)
                exceed100_df_all = pd.concat([exceed100_df_all,exceed100_df_this])
                removed100_df_this = parse_latlons(stage4_this,results['removed_data1_exceed100'],stage4_exceed100_df,event_num,time_pd)
                removed100_df_all = pd.concat([removed100_df_all,removed100_df_this])

            if stage4_exceed1000_df is not None:
                exceed1000_df_this = parse_latlons(stage4_this,results['data1_exceed1000'],stage4_exceed1000_df,event_num,time_pd)
                exceed1000_df_all = pd.concat([exceed1000_df_all,exceed1000_df_this])
                removed1000_df_this = parse_latlons(stage4_this,results['removed_data1_exceed1000'],stage4_exceed1000_df,event_num,time_pd)
